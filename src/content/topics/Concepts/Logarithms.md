...
The natural logarithm, or natural log, is simply the logarithm with the base of Euler's constant, $e$.
$$
\log_e(x)
$$
...
In the West, the common notation is to infer that $\log(x)$ is $\log_{10}(x)$ and use the alternate phrase $\ln(x)$ specifically for the natural log $\log_{e}(x)$. Everywhere else the standard notation is to use $\log(x)$ to mean $\log_{e}(x)$ and then write an explicit base if they mean to use any other base.

In effect many articles will either specifically express that $\log(x)$ is used as the natural log, or instead use $\ln$ and disregard the contradiction. Mathematicians are also generally flexible while reading to adapt to either notation. Regardless of which notation you choose to use, it is most important that your work is clearly legible and documented.